...
public record RenameEnvironmentSet(Guid EnvironmentSetId, string newName) : IRequest;
...
public class RenameEnvironmentSetHandler : IRequestHandler<RenameEnvironmentSet>
{
    ...
    public async Task Handle(RenameEnvironmentSet command, CancellationToken cancellationToken)
    {
        ...
        await _documentSession.AppendToStreamAndSave<EnvironmentSet>(command.EnvironmentSetId, new EnvironmentSetRenamed(command.EnvironmentSetId, command.newName), _userInfo.GetCurrentUserId());
    }
}