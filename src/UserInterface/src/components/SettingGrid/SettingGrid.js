...
const SettingsGrid = ({ transformedSettings, onAddSetting, onSettingChange, onSettingRename, onSettingDelete, onEnvironmentRename, onDeleteEnvironment, showEditButtons }) => {
    ...
    const handleConfirm = () => {
        setDialogOpen(false);
        onSettingDelete(itemToDelete);

        if (itemToDelete in settings.settings) {
            const updatedSettings = { ...settings };// Create a shallow copy of the settings object
            delete updatedSettings.settings[itemToDelete];        // Delete the setting from the copy
            setSettings(updatedSettings);
        }
    };
